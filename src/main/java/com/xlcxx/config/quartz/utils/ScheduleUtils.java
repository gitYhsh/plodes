...
public class ScheduleUtils {
    ...
    private static TriggerKey getTriggerKey(Long jobId) {
        return TriggerKey.triggerKey(Constant.JOB_NAME + jobId);
    }
    ...
    private static JobKey getJobKey(Long jobId) {
        return JobKey.jobKey(Constant.JOB_NAME + jobId);
    }
    ...
    public static void createScheduleJob(Scheduler scheduler, Job scheduleJob) {
        try {
            ...
            // 暂停任务
            if (StringUtils.equals(scheduleJob.getStatus(), Constant.JOB_PASUE)) {
